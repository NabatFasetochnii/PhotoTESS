...
from Headers import Get_Times
from Postproc import Post_Proc
from TTF import TTF_Query
from Utils import get_com, unzip, print_info

warnings.simplefilter("ignore")


def photometry(path2data=r'G:\tess\good\sent\TIC22951086601\3988\EAST_V', RAper=None, observer='chazov'):
    if RAper is None:
        RAper = [3, 4, 5]
    tess_object = None
    ...
    Bbox = 5  # centering/morphology box half-size (pixels)
    ...
    # Pref = DT + '_Kourovka0.4m_' + Header['FILTER']

    # draw and save picture
    Pic_Name = Path2Save + '/' + tess_object + '_Kourovka0.4_' + \
               Header['FILTER'] + '_' + \
               Times[0].datetime.strftime('%Y-%m-%d') + '_field.pdf'
    Draw_Map(Data, Pic_size, Header, Catalog, Pic_Name, RAper[0], tess_object, DT)

    # open log file
    df = open(Path2Save + '/Time.txt', 'w')
    df.write('DATE-OBS\tJD\tHJD\tBJD\t' +
             'EXPTIME\tFILTER\tAIRMASS\tEXTINCT\t' +
             'SKY-TEMP\tSEXFWHM\tSEXELL\tZEROPOI\t' +
             'Sky\tX\tY\tMax\n')
    ...
    for f in file_list:
        ...
        #     plt.imshow(Data, vmin = 0, vmax = 1000, cmap='gray_r')
        #     Stellar_aper[0].plot(color='b', lw=1.5)
        #     plt.show()
    ...
    Post_Proc.post_proc(apers=RAper, path2data=Path2Save, observer=observer)
